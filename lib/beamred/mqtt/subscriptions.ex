defmodule BeamRED.MQTT.Subscriptions do
  alias BeamRED.MQTT.Topics
  ...
  def subscribe(subscriptions, pid, topics) do
    with {:ok, parsed_topics} <- parse_subscribed_topics(topics) do
      new_set = MapSet.new(parsed_topics)

      Map.update(subscriptions, pid, new_set, fn existing_set ->
        MapSet.union(existing_set, new_set)
      end)
    else
      :error -> :error
    end
  ...
  """
  @spec unsubscribe(subscriptions(), pid(), [String.t()] | :all) ::
  ...
  def unsubscribe(subscriptions, pid, topics) do
    with {:ok, parsed_topics} <- parse_subscribed_topics(topics),
         {:ok, existing_topics} <- Map.fetch(subscriptions, pid) do
      {_, updated} =
        parsed_topics
        |> Enum.map_reduce(existing_topics, fn item, acc ->
  ...
  def list_matched(subscriptions, topic) do
    case Topics.parse_published_topic(topic) do
      {:ok, published_topic} ->
        subscriptions
        |> Stream.flat_map(fn {pid, subscribed_topics} ->
          Enum.map(subscribed_topics, fn t -> {pid, t} end)
        end)
